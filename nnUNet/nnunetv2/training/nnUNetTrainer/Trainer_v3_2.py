...
class Trainer_v3_2(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True, device: torch.device = torch.device('cuda')):
        ...
        self.layers_to_freeze = ["_orig_mod.encoder.stages.0","_orig_mod.encoder.stages.1","_orig_mod.encoder.stages.2","_orig_mod.encoder.stages.3","_orig_mod.encoder.stages.4","_orig_mod.encoder.stages.5"]#,"_orig_mod.decoder.stages.0","_orig_mod.decoder.stages.1"],"_orig_mod.decoder.stages.2"]
