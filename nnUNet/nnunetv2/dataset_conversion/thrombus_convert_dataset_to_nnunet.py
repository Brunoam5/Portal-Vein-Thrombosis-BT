...
def generate_json_from_dir_v2(foldername, subjects_train, subjects_val, labels):
    ...
    json_dict = {
        'name': "Own_Dataset_Thrombus_PVT",
        'description': "Segmentation of own cases of portal vein thrombosis",
        'channel_names': {"0": "CT"},
        'labels': {"background": 0, "portal_vein_and_splenic_vein": 1, "thrombus": 2},  # Only thrombus included in labels
...
def combine_labels(ref_img, file_out, masks, labels):
    ...
    # Only process the 'thrombus' label mask
    thrombus_mask = Path(masks / "thrombus.nii.gz")
    
    if thrombus_mask.exists():
        img = nib.load(thrombus_mask)
        combined[img.get_fdata() > 0] = 2  # Set thrombus mask to label 2
    else:
        print(f"Missing: {thrombus_mask}")
...
if __name__ == "__main__":
    dataset_path = Path("/gpfs/home/bandres/nnUNetDataset/Database_Auto_Seg")
    nnunet_path = Path("/gpfs/home/bandres/nnUNetFrame/dataset/nnUNet_raw/Dataset003_ThrombusDataset_v2")
